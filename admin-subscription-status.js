...
(function(){
  ...
  async function load(){
    try{
      ...
      const t=await window.supabase.from('tenants').select('id,status').eq('tenant_code',code).maybeSingle();
      if(t.error||!t.data)return;
      const [r,p]=await Promise.all([
        window.supabase.from('jfs_laundry_subscription_requests').select('plan_id,status,activated_at,requested_at').eq('tenant_id',t.data.id).order('requested_at',{ascending:false}).limit(20),
  ...
  function mount(){
    if(document.getElementById('jfs-sub-card'))return;
