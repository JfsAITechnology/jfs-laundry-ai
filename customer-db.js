...
function dropoffAnswer(){const t=window.activeConfig||window.dbTenant||{};const name=t.name||t.business_name||'laundry';const address=t.address||[t.city,t.province].filter(Boolean).join(', ');if(!address||address==='Alamat laundry')return null;return`Untuk antar sendiri, Kakak bisa langsung membawa cucian ke ${name} di alamat berikut:\n📍 ${address}\n\nAlamat ini diambil dari data resmi laundry. Silakan pilih “Antar Sendiri” pada form “Pesan Laundry”.`}
async function kbSendMessage(){
  ...
  try{
    ...
    if(has(q,['antar sendiri','antar cucian','drop off','dropoff','datang ke toko'])){const direct=dropoffAnswer();if(direct){addBubble(direct);pushChat('assistant',direct);return}}
