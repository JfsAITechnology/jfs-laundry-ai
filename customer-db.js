...
async function getKbTenant(){
  ...
  const{data}=await jfsDb.from('jfs_public_tenant_catalog').select('tenant_code,business_name,address,city,province,whatsapp,phone,ai_enabled').eq('tenant_code',id).maybeSingle();
  window.dbTenant=data||null;return window.dbTenant;
}
...
async function getProducts(){
  ...
  const{data,error}=await jfsDb.from('jfs_public_products').select('name,description,price,category').eq('tenant_code',t.tenant_code).gt('price',0).order('name');
...
async function knowledgeAnswer(question){
  ...
  const[k,p]=await Promise.all([
    jfsDb.from('jfs_public_knowledge').select('category,title,content').eq('tenant_code',t.tenant_code),
    jfsDb.from('jfs_public_products').select('name,description,price,category').eq('tenant_code',t.tenant_code)
  ]);
  const docs=[
    ...(k.data||[]).map(x=>({content:x.content,score:score(question,`${x.category||''} ${x.title||''} ${x.content||''}`)})),
    ...(p.data||[]).map(x=>({content:`${x.name}: ${x.description||''}${x.price?` Harga ${rupiah(x.price)}.`:''}`,score:score(question,`${x.name} ${x.description||''} ${x.category||''}`)}))
...
function handoverAnswer(label='KOMPLAIN'){
  const t=window.activeConfig||{};
  const wa=t.waNumber?` Silakan hubungi admin melalui tombol WA Toko agar dapat ditangani langsung.`:'';
...
async function kbSendMessage(){
  ...
  try{
    ...
    // PRIORITAS 1 — komplain / kondisi khusus: jangan jawab dengan asumsi AI.
    if(has(q,['komplain','keluhan','hilang','kehilangan','rusak','sobek','luntur','tertukar','kurang','telat','terlambat','salah cuci','bau','warna berubah','ganti rugi','refund','uang kembali','protes'])){
      const a=handoverAnswer('KOMPLAIN');addBubble(a);pushChat('assistant',a);return;
    }

    // PRIORITAS 2 — status pesanan: gunakan jalur tracking resmi, jangan menebak status.
    if(has(q,['status pesanan','status order','lacak pesanan','tracking','pesanan saya','order saya'])){
      const a=statusGuide();addBubble(a);pushChat('assistant',a);return;
    }

    // PRIORITAS 3 — pemesanan: arahkan ke form agar order tervalidasi oleh server.
    if(has(q,['pesan','order','booking','pesan laundry','mau cuci','mau laundry'])){
      const a=orderGuide();addBubble(a);pushChat('assistant',a);return;
    }

    // PRIORITAS 4 — harga/tarif: selalu ambil dari database resmi, bukan pengetahuan model.
    if(/\b(harga|tarif|price|berapa|biaya)\b/.test(q)){
      const direct=await priceAnswer();
      if(direct){addBubble(direct);pushChat('assistant',direct);return;}
      const a='Saya belum menemukan tarif resmi di data laundry. Agar tidak memberi harga yang salah, silakan hubungi toko melalui WhatsApp.';addBubble(a);pushChat('assistant',a);return;
    }

    // PRIORITAS 5 — estimasi: jangan mengarang total. Minta data yang dibutuhkan.
    if(has(q,['estimasi','perkiraan total','total biaya','hitung total','kira kira total','kira-kira total'])){
      const weight=q.match(/(\d+(?:[.,]\d+)?)\s*(kg|kilo|kilogram)/i);
      if(!weight){const a='Bisa Kak. Untuk estimasi, berapa perkiraan berat cucian dalam Kg? Setelah itu saya hitungkan berdasarkan tarif resmi yang tersedia.';addBubble(a);pushChat('assistant',a);return;}
      const a='Berat sudah dicatat sebagai perkiraan. Untuk menghitung total dengan benar, pilih layanan yang akan digunakan pada form “Pesan Laundry”. Total di form dihitung dari tarif resmi dan jumlah Kg. Jika ada ketentuan area/waktu jemput khusus, admin akan memeriksanya sebelum pelaksanaan.';addBubble(a);pushChat('assistant',a);return;
    }

    // PRIORITAS 6 — pickup: jangan menjanjikan jam sebelum ada slot resmi.
    if(has(q,['jemput','pickup','ambil cucian','waktu jemput','jam jemput'])){
      const a='Kakak bisa memilih “Jemput Cucian” pada form. Waktu yang dipilih adalah permintaan waktu jemput; bukan jaminan jadwal sampai dikonfirmasi oleh sistem/admin.';addBubble(a);pushChat('assistant',a);return;
    }

    // Layanan spesifik dari data resmi.
    const service=await serviceAnswer(q);if(service){addBubble(service);pushChat('assistant',service);return;}

    // Knowledge Base resmi.
    let a=await knowledgeAnswer(text);
    if(!a)a='Saya belum menemukan informasi itu di data resmi laundry. Agar tidak memberi informasi yang salah, silakan hubungi toko melalui WhatsApp untuk pertanyaan khusus.';
    addBubble(a);pushChat('assistant',a);
  }catch(e){
    console.error('Chatbot error',e);
    const a='Maaf Kak, data layanan sedang tidak dapat dimuat. Saya tidak mau menebak informasinya. Silakan coba lagi atau hubungi toko melalui WhatsApp.';addBubble(a);pushChat('assistant',a);
  }
}
